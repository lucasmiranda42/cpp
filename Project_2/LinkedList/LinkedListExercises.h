...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  if (!head_) {
    ...
    // If the first node is bigger than the new one, then we have a new head
    // We should append the new node at the begining
    Node* old_head = head_;
    ...
    // If the last node is smaller than the new one, then we have a new tail
    // We should append the new node at the end
    Node* old_tail = tail_;
    ...
    // Otherwise, iterate over the list until you find a value greater than the new node    
    Node* prev = nullptr; // pointer to previous node than current
    Node* cur = head_; // pointer to current node
